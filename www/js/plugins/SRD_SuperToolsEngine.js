...
Imported["SumRndmDde Super Tools Engine"] = 1.17;
...
WindowManager.windowMeetsConditions = function(window) {
	return window && window.isOpen() && window.visible && window.backOpacity > 0 && window.contentsOpacity > 0;
};
...
Scene_Base.prototype.initCustomWindowPositions = function() {
	...
	for(let i = 0; i < children.length; i++) {
		...
		if(win && $dataWindows[this.constructor.name][win.constructor.name]) {
			...
			const errorText = 'Error with ' + this.constructor.name + '\'s ' + win.constructor.name;
